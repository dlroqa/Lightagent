//! Where the Lightagent CLI lives on disk, and how it is replaced safely.
...
const REPOSITORY: &str = "https://github.com/dlroqa/Lightagent.git";

/// Why the binary at `executable` must not be replaced in place, if it must not.
///
/// Both refusals are read from the path, not the environment: an application
/// bundle may ship its CLI in `resources/bin`, and a Cargo `target/`
...
pub(crate) fn managed_install(executable: &Path) -> Option<String> {
    ...
    {
        return Some(format!(
            "{} is bundled with an application; update that application instead",
            executable.display()
        ));
    }
    if dirs
...
/// The directory where Lightagent is installed.
///
...
/// The version the installed binary reports through `--version`, or `None` when
...
/// The binary replaced by an update, with the previous copy of each
/// kept aside until the whole update has succeeded.
///
/// The previous binary remains available until the replacement is verified,
/// allowing any failure to restore the original installation.
#[derive(Default)]
pub(crate) struct Transaction {
    /// The installed path, and where its previous binary was set aside (`None`
...
mod tests {
    ...
    #[test]
    fn bundled_and_development_binaries_are_not_replaced() {
        for path in [
            "/Applications/Lightagent.app/Contents/Resources/bin/lightagent",
            "/tmp/.mount_Lightagent/resources/bin/lightagent",
            "C:/Program Files/Lightagent/resources/bin/lightagent.exe",
            "/srv/src/Lightagent/target/debug/lightagent",
            "/srv/src/Lightagent/target/release/lightagent",
        ] {
            assert!(managed_install(Path::new(path)).is_some(), "{path}");
        }
        for path in [
            "/usr/local/bin/lightagent",
            "/opt/cargo/bin/lightagent",
            "/opt/lightagent-0.3.22-x86_64-unknown-linux-gnu/lightagent",
    ...
    fn version_output_is_read_only_from_the_expected_binary() {
        ...
        assert_eq!(
            parse_version_output("another-cli 0.3.22", Cli::Lightagent),
            None
        );
    ...
    fn archive_names_follow_the_packaging_scripts() {
        let name = archive_name(Cli::Lightagent, "0.3.22");
        assert!(name.starts_with("lightagent-0.3.22-"));
    ...
    fn committed_binaries_stay_and_their_old_copies_are_removed() {
        ...
        for name in ["lightagent-a", "lightagent-b"] {
    ...
    #[test]
    fn a_later_install_failure_restores_the_first_change() {
        let temp = TempDir::new("rollback");
        let first = temp.0.join("lightagent-a");
        let second = temp.0.join("lightagent-b");
        std::fs::write(&first, b"old-a").unwrap();
        std::fs::write(temp.0.join("lightagent-a.new"), b"new").unwrap();
        std::fs::write(&second, b"old-b").unwrap();

        let mut transaction = Transaction::default();
        transaction
            .install_staged(&temp.0.join("lightagent-a.new"), &first)
            .unwrap();
        assert_eq!(std::fs::read(&first).unwrap(), b"new");
        // The second staged path is absent, so the transaction fails after the first change.
        let error = transaction
            .install_staged(&temp.0.join("lightagent-b.new"), &second)
            .unwrap_err();
        assert!(error.contains("could not install"));
        assert_eq!(std::fs::read(&second).unwrap(), b"old-b");
    ...
    fn a_preserved_binary_is_restored_after_it_was_rebuilt_in_place() {
        ...
        let target = temp.0.join("lightagent");
        std::fs::write(&target, b"old").unwrap();
        let mut transaction = Transaction::default();
