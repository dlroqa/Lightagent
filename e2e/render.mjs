// Render the standalone Lightagent Web UI against a live harness API.
// The assertions are behavioural rather than pixel-exact so platform font
// rendering cannot make the check flaky.

import { mkdir } from "node:fs/promises";
import { chromium } from "playwright";

const BASE = (process.env.PANEL_BASE ?? "http://127.0.0.1:8735").replace(/\/+$/, "");
...
// A misconfigured static host can return the SPA document for API requests.
// Show a useful remedy, then prove Retry recovers against the harness endpoint.
  ...
  for (const failure of [
    {
      ...
      expected: "Lightagent server returned an unexpected response",
    },
    {
      status: 502,
