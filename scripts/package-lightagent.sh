...
#
# Lightagent is an independent agent harness. The archive carries its CLI and
# compiled Web UI; inference remains an external OpenAI-compatible service.
# The service wrapper in
...
[ -f frontend/dist/index.html ] || {
  echo "frontend/dist is missing; run npm run build --prefix frontend first" >&2
  exit 1
}
...
# that Lightagent needs a provider and how to launch the Web UI are the essentials.
cat > "$STAGE/README.md" <<EOF
# Lightagent $VERSION — $TRIPLE

A local agent harness with live tools and a standalone Web UI. Run
\`./lightagent\` to open terminal
...
- **Lightagent needs a running inference provider.** It talks to an
  OpenAI-compatible endpoint over HTTP; it does not hold
...
- **The Web UI is included.** Run
  \`./lightagent serve --web-root ./web\` and open the printed address.
...
case "$TRIPLE" in
  *windows*)
    ...
    # malformed zip) — this keeps the release portable across runners.
